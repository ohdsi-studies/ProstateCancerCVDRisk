...
#    # cmAnalysisList[1] "Intent-to-treat / 1:1 matching"
#    # cmAnalysisList[3] "Intent-to-treat / 1:100 variable ratio matching"
#  }
#  if (timeAtRisk == "IntentToTreat") {
#    cmAnalysisList <- cmAnalysisList[c(1,3)]
#    # cmAnalysisList[2] "As-treated / 1:1 matching"
#    # cmAnalysisList[4] "As-treated / 1:100 variable ratio matching"
#  }

# The code below was added by Mitch on Dec-20-2019
...
# The code above was added by Mitch on Dec-20-2019
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "EPI720")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "EPI720")
...
exportDiagnostics <- function(outputFolder,
  ...
  for (i in 1:length(files)) {
    ...
    #added to fix Table 1 bug where rows are missing
    ...
    #Commented out to fix Table 1 bug where rows are missing
