...
synthesizePositiveControls <- function(connectionDetails,
  ...
  if (!file.exists(synthesisSummaryFile)) {
    ...
    pathToJson <- system.file("settings", paste0("positiveControlSynthArgs", timeAtRisk, ".json"), package = "EPI720") 
    #pathToJson <- system.file("settings", "positiveControlSynthArgs.json", package = "EPI720")
